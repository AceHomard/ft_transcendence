...
function PlayFakeGame()
{
    if (isAnimating == false)
    {
        ...
        isAnimating = true;
        animationFrameId = window.requestAnimationFrame(playPongAnimation);
        

        canvas = document.getElementById("play_page_canvas");
        ctx = canvas.getContext("2d");

        cwidth = width * resolution;
        cheight = height * resolution;

        canvas.width = cwidth;
        canvas.height = cheight;
    }
    //console.log(ball_pose.velocity);
}

function CancelFakeGame()
{
    if (isAnimating)
    {
