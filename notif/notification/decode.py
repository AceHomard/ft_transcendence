...
from notification.models import TransactionId
...
def check_transaction_id(transaction_id):
    if TransactionId.objects.filter(transaction_id=transaction_id).exists():
        print("[API] Key", "Transaction ID already exists")
        return False
    try:
...
def decrypt(message, signature, public_key):
    try:
        ...
        print("[API] GET", "Token Valid")
        return True
    except Exception as e:
        print("[API] GET", "Token Invalid")
        return False
...
def decrypt_routine(request):
    ...
    public_key = load_public_key("keynotif/public_key.pem")
